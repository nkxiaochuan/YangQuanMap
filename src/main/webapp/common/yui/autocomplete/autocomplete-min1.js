...
YAHOO.widget.AutoComplete.prototype._toggleContainer = function (bShow) {
	...
	if (oAnim && oAnim.getEl() && (this.animHoriz || this.animVert)) {
		...
		oAnim.attributes = (bShow) ? {width:{to:wExp}, height:{to:hExp}} : {width:{to:wColl}, height:{to:hColl}};
		if (bShow && !this._bContainerOpen) {
		...
		if (scrollOn && (nNewItemIndex > -1) && (nNewItemIndex < this._nDisplayedItems)) {
			if (nKeyCode == 40) {
				if ((oNewItem.offsetTop + oNewItem.offsetHeight) > (oContent.scrollTop + oContent.offsetHeight)) {
					oContent.scrollTop = (oNewItem.offsetTop + oNewItem.offsetHeight) - oContent.offsetHeight + 36;
				} else {
					if ((oNewItem.offsetTop + oNewItem.offsetHeight) < oContent.scrollTop) {
				...
				if (oNewItem.offsetTop < oContent.scrollTop) {
					...
					if (oNewItem.offsetTop > (oContent.scrollTop + oContent.offsetHeight)) {
